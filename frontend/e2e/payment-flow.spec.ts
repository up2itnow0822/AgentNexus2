...
test.describe('Payment Flow', () => {
  ...
  test('should show agent price on detail page', async ({ page }) => {
    await page.goto('/');
    ...
    // Verify price on detail page
    const priceElement = page.locator('[data-testid="agent-detail-price"]');
    await expect(priceElement).toBeVisible();
  ...
  test('should show purchase button for unpurchased agents', async ({ page }) => {
    await page.goto('/');
    await page.waitForSelector('[data-testid="agent-card"]');
    ...
    // Either purchase button exists or "Execute" button (if already owned)
    ...
    expect(hasPurchaseButton || hasExecuteButton).toBeTruthy();
  });
  ...
  test('should update UI after successful purchase', async ({ page }) => {
    ...
    const hasProfileContent = await page.locator('[data-testid="profile-content"], text=/purchased|agents|history/i').isVisible().catch(() => false);
    
    expect(hasProfileContent).toBeTruthy();
  });

  test('should show purchased agents in profile', async ({ page }) => {
    await page.goto('/profile');
    
    await page.waitForTimeout(1000);
    
    // Check for purchased agents section
    ...
    // Either section exists or page shows empty state
    const hasEmptyState = await page.locator('text=/no agents|empty/i').isVisible().catch(() => false);
    
    expect(hasPurchasedSection || hasAgentsList || hasEmptyState).toBeTruthy();
  });
...
test.describe('Smart Contract Integration', () => {
  ...
  test('should show transaction confirmation', async ({ page }) => {
    ...
    const hasProfile = await page.locator('[data-testid="profile-content"]').isVisible().catch(() => false) ||
                       await page.locator('text=/profile|purchased/i').isVisible().catch(() => false);
