...
test.describe('Loading States', () => {
  ...
  test.skip('should show loading indicator while fetching agents', async ({ page }) => {
    await page.goto('/', { waitUntil: 'domcontentloaded' });

    // Wait for page to settle
    await page.waitForTimeout(3000);

    // Check for loading state, agents, or main content
    ...
    // Either loading state appeared, agents loaded, or main content is visible
    expect(hasLoading || hasAgents || hasAgentGrid || hasMainContent).toBeTruthy();
  });
...
test.describe('Empty States', () => {
  ...
  test('should show empty state when no agents available', async ({ page, context }) => {
    ...
    // Should show empty state message or data-testid
    const hasEmptyState = await page.locator('[data-testid="empty-agents"]').isVisible().catch(() => false);
    const hasEmptyText = await page.locator('text=/no agents|empty|get started|create agent|discover/i').isVisible({ timeout: 5000 }).catch(() => false);
    const hasAgentGrid = await page.locator('[data-testid="agent-grid"]').isVisible().catch(() => false);

    expect(hasEmptyState || hasEmptyText || hasAgentGrid).toBeTruthy();
  });

  test.skip('should show empty state in profile when no purchases', async ({ page }) => {
    await page.goto('/profile');

    await page.waitForTimeout(2000);

    // Should show either purchases, empty state, or wallet connect prompt
    const hasPurchases = await page.locator('[data-testid="purchased-agents"]').isVisible().catch(() => false);
    const hasEmptyState = await page.locator('[data-testid="empty-purchases"]').isVisible().catch(() => false);
    const hasEmptyText = await page.locator('text=/no agents|haven\'t purchased|get started/i').isVisible().catch(() => false);
    const hasWalletPrompt = await page.locator('text=/connect wallet|connect your wallet/i').isVisible().catch(() => false);

    expect(hasPurchases || hasEmptyState || hasEmptyText || hasWalletPrompt).toBeTruthy();
  });
...
test.describe('Performance', () => {
  ...
  test('should load homepage quickly', async ({ page }) => {
    ...
    // Should load within 10 seconds (relaxed for test parallelism)
...
test.describe('Interactive Elements', () => {
  ...
  test.skip('should have clickable cards', async ({ page }) => {
    await page.goto('/');
    ...
    if (await agentCard.isVisible()) {
      ...
      // URL should change to agent detail page
      ...
      expect(newUrl !== url || hasModal || hasAgentDetail).toBeTruthy();
    } else {
      // No agents visible - test passes (empty state is valid)
      expect(true).toBeTruthy();
    }
  });
